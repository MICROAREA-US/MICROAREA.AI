...
// WP-Optimize advanced-cache.php (written by version: 3.5.0) (homeurl: http://localhost:8000/) (abspath: C:\wamp64\www\microarea.ai/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'C:\wamp64\www\microarea.ai\wp-content\plugins\wp-optimize\cache',
);
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'C:\wamp64\www\microarea.ai\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-microarea.ai.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-microarea.ai.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-microarea.ai.php');
}
